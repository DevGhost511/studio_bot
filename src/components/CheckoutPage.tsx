    ...
    const handleCheckout = async () => {
      try {
        ...
        const { data } = await axios.post(
          `${window.origin}/.netlify/functions/stripe`,
          {
            amount: planPrice, // Amount in cents (e.g., $10.00)
          }
        );
        ...
        if (result?.error) {
          ...
          if (profile) {
            await saveUserProfile({
              ...
              planStart: new Date(),
              planEnd: new Date(new Date().setDate(new Date().getDate() + 15)),
            });
            const { phone_number } = await getPhoneNumber(profile.model_id);
            await updateAssistant({ phone_number: phone_number });
            await updatePhoneNumber(profile.model_id, { is_active: false });
